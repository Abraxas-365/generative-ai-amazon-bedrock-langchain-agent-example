...
def elicit_slot(
    session_attributes, active_contexts, intent, slot_to_elicit, message
):
    response = {
        "sessionState": {
            "activeContexts": [
...
def close(
    session_attributes, active_contexts, fulfillment_state, intent, message
):
    response = {
        "sessionState": {
            "activeContexts": [
            ...
            {"contentType": "PlainText", "content": message},
            {
                ...
                "imageResponseCard": {
                    "buttons": [
                        ...
                        {"text": "Loan Calculator", "value": "Loan Calculator"},
                        {
                            "text": "Ask GenAI",
...
def isvalid_yes_or_no(word):
    ...
    similarity_scores = [
        difflib.SequenceMatcher(None, word.lower(), ref_word).ratio()
...
def isvalid_pin(userName, pin):
    ...
    try:
        ...
        params = {
            ...
            "ExpressionAttributeValues": {":c": userName},
        }
...
def validate_pin(intent_request, slots):
    ...
    if username is not None:
        ...
        session_attributes = (
            intent_request["sessionState"].get("sessionAttributes") or {}
        )
    ...
    if pin is not None:
        if not isvalid_pin(username, pin):
            return build_validation_result(
                False,
                "Pin",
                "You have entered an incorrect PIN. Please try again.".format(
                    pin
                ),
            )
    else:
        message = "Thank you for choosing AnyCompany, {}. Please confirm your 4-digit PIN before we proceed.".format(
            username
        )
        return build_validation_result(False, "Pin", message)
...
def verify_identity(intent_request):
    ...
    confirmation_status = intent_request["sessionState"]["intent"][
        "confirmationState"
    ]
    session_attributes = (
        intent_request["sessionState"].get("sessionAttributes") or {}
    ...
    validation_result = validate_pin(
        intent_request, intent_request["sessionState"]["intent"]["slots"]
    )
    session_attributes["UserName"] = username
            ...
            try:
                ...
                for item in items:
                    if item["planName"].lower() == "mortgage":
                        message = "Your mortgage account summary includes a ${:,.2f} loan at {}% interest with ${:,.2f} of unpaid principal. Your next payment of ${:,.2f} is scheduled for {}.".format(
                            float(item["loanAmount"]),
                            float(item["loanInterest"]),
                            float(item["unpaidPrincipal"]),
                            float(item["amountDue"]),
                            item["dueDate"],
                        )
                    elif item["planName"].lower() == "checking":
                        message = "I see you have a Checking account with AnyCompany. Your account balance is ${:,.2f} and your next payment amount of ${:,.2f} is scheduled for {}.".format(
                            float(item["unpaidPrincipal"]),
                            float(item["paymentAmount"]),
                            item["dueDate"],
                        )
                    elif item["planName"].lower() == "loan":
                        message = "I see you have a Loan account with AnyCompany. Your account balance is ${:,.2f} and your next payment amount of ${:,.2f} is scheduled for {}.".format(
                            float(item["unpaidPrincipal"]),
                            float(item["paymentAmount"]),
                            item["dueDate"],
                        )
                ...
                return elicit_intent(
                    intent_request,
                    session_attributes,
                    "Thank you for confirming your username and PIN, {}. {}".format(
                        username, message
                    ),
                )
...
def validate_loan_application(intent_request, slots):
    ...
    if username is not None:
        ...
        try:
            session_username = intent_request["sessionState"][
                "sessionAttributes"
            ]["UserName"]
    ...
    if loan_value is not None:
        if loan_value.isnumeric():
            if not isvalid_zero_or_greater(loan_value):
                return build_validation_result(
                    False, "LoanValue", "Please enter a value greater than $0."
                )
        else:
            prompt = (
                "The user was just asked to provide their loan value on a loan application and this was their response: "
    ...
    if monthly_income is not None:
        if monthly_income.isnumeric():
            ...
            prompt = (
                "The user was just asked to provide their monthly income on a loan application and this was their response: "
                + intent_request["inputTranscript"]
            )
            message = invoke_fm(prompt)
        ...
        return build_validation_result(
            False, "MonthlyIncome", "What is your monthly income?"
        )

    if work_history is not None:
        if not isvalid_yes_or_no(work_history):
            prompt = (
                "The user was just asked to confirm their continuous two year work history on a loan application and this was their response: "
                + intent_request["inputTranscript"]
            )
            message = invoke_fm(prompt)
            reply = (
    ...
    if credit_score is not None:
        if credit_score.isnumeric():
            ...
            prompt = (
                "The user was just asked to provide their credit score on a loan application and this was their response: "
                + intent_request["inputTranscript"]
            )
            message = invoke_fm(prompt)
            reply = (
    ...
    if housing_expense is not None:
        if housing_expense.isnumeric():
            ...
            prompt = (
                "The user was just asked to provide their monthly housing expense on a loan application and this was their response: "
                + intent_request["inputTranscript"]
            )
            message = invoke_fm(prompt)
            reply = (
    ...
    if debt_amount is not None:
        if debt_amount.isnumeric():
            ...
            prompt = (
                "The user was just asked to provide their monthly debt amount on a loan application and this was their response: "
                + intent_request["inputTranscript"]
            )
            message = invoke_fm(prompt)
            reply = (
    ...
    if down_payment is not None:
        if down_payment.isnumeric():
            ...
            prompt = (
                "The user was just asked to provide their estimated down payment on a loan application and this was their response: "
                + intent_request["inputTranscript"]
            )
            message = invoke_fm(prompt)
        ...
        return build_validation_result(
            False, "DownPayment", "What do you have saved for a down payment?"
        )

    if coborrow is not None:
        if not isvalid_yes_or_no(coborrow):
            prompt = (
                "The user was just asked to confirm if they will have a co-borrow on a loan application and this was their response: "
                + intent_request["inputTranscript"]
            )
            message = invoke_fm(prompt)
        ...
        return build_validation_result(
            False, "Coborrow", "Do you have a co-borrower?"
        )

    if closing_date is not None:
        if not isvalid_date(closing_date):
            prompt = (
                "The user was just asked to provide their real estate closing date on a loan application and this was their response: "
                + intent_request["inputTranscript"]
            )
            message = invoke_fm(prompt)
        ...
        print("## ClosingDate")
        return build_validation_result(
            False, "ClosingDate", "When are you looking to close?"
        )
...
def loan_application(intent_request):
    ...
    if intent_request["invocationSource"] == "DialogCodeHook":
        # Validate any slots which have been specified. If any are invalid, re-elicit for their value
        validation_result = validate_loan_application(
            intent_request, intent_request["sessionState"]["intent"]["slots"]
        )
        print(
            "LOAN APPLICATION - validation_result = " + str(validation_result)
        )
        if "isValid" in validation_result:
            if validation_result["isValid"] == False:
                if (
                    validation_result["violatedSlot"] == "CreditScore"
                    and confirmation_status == "Denied"
                ):
    ...
    if username and monthly_income:
        ...
        s3_client.download_file(
            s3_artifact_bucket,
            "agent/assets/Mortgage-Loan-Application.pdf",
        ...
        writer.addpage(
            reader.pages[0]
        )  # Assuming you're updating the first page

        print("writing to output file")
        with open(
            "/tmp/Mortgage-Loan-Application-Completed.pdf", "wb"
        ) as output_stream:
            writer.write(output_stream)

        s3_client.upload_file(
            "/tmp/Mortgage-Loan-Application-Completed.pdf",
            s3_artifact_bucket,
        ...
        mortgage_app = (
            "Your loan application is nearly complete! Please follow the link for the last few bits of information: "
            + URLs[0]
        )

        return elicit_intent(intent_request, session_attributes, mortgage_app)
